...
void convexHull3 (Points &points, vector<int> &hull)
{
	...
	for (int i = 4; i < arr.vertices.size(); ++i) {
		...
		Vertices vertices;
		listUpdateVertices(arr, horizon, vertices);
...
void listUpdateVertices(Arrangement &arr, Edges &horizon, Vertices &vertices)
{
	for (int i = 0; i < horizon.size(); ++i) {
		...
		vertices.insert(vertices.end(), f1->visibleVertices.begin(), f1->visibleVertices.end());
		vertices.insert(vertices.end(), f2->visibleVertices.begin(), f2->visibleVertices.end());
	}
}
